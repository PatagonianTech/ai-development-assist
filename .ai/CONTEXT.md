...
## AI Configuration (`.ai/` Directory)

The `.ai/` folder centralizes all AI assistant configuration and commands. Its purpose is to provide a unified, IDE-agnostic location for AI context and slash commands that can be symlinked to different AI tools (Cursor, Claude, Windsurf).

### Structure

```
.ai/
├── CONTEXT.md              # Main AI context file with project guidance
└── commands/               # Slash commands for AI assistants
    ├── design.md           # /design - Create implementation plans from Jira tickets
    ├── implement.md        # /implement - Execute the design plan
    ├── resume.md           # /resume - Continue work on existing tickets
    ├── changelog.md        # /changelog - Update changelog after manual changes
    └── git/                # Git-related commands
        ├── commit.md       # /git:commit - Commit current changes
        ├── cpush.md        # /git:cpush - Commit and push changes
        └── codereview.md   # /git:codereview - Review code changes
```

### Symlinks

The contents of `.ai/` are symlinked to multiple IDE-specific locations:
- `.ai/CONTEXT.md` → `CLAUDE.md`, `.cursor/rules/config.mdc`, `.windsurfrules`
- `.ai/commands/` → `.claude/commands/`, `.cursor/commands/`

This allows the same AI configuration to work across different AI development tools while maintaining a single source of truth.
